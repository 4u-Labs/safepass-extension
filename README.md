# 🛡️ SafePass Extension — Gerenciador Zero-Knowledge
### **Cofre de Senhas Criptografado com PBKDF2 + AES-GCM 256 no Navegador**

<p align="center">
  <img src="https://img.shields.io/badge/Manifest-V3-4285F4?style=for-the-badge&logo=google-chrome&logoColor=white" alt="Manifest V3" />
  <img src="https://img.shields.io/badge/Chrome_Extension-Official-10b981?style=for-the-badge&logo=google-chrome&logoColor=white" alt="Chrome Extension" />
  <img src="https://img.shields.io/badge/Privacidade-100%25_Local-6d4aff?style=for-the-badge" alt="Privacidade" />
  <a href="https://4u.ia.br/app/safepass/"><img src="https://img.shields.io/badge/Web_App-4U.IA.BR-0ea5e9?style=for-the-badge&logo=google-chrome&logoColor=white" alt="Web App" /></a>
</p>

---

## 📖 Visão Geral

Extensão Chrome MV3 do SafePass com preenchimento automático inteligente, geração de senhas de alta entropia e arquitetura estrita de Conhecimento Zero (Zero-Knowledge).

Parte integrante do ecossistema de soluções e sistemas web de alta performance da **[4U.IA.BR](https://4u.ia.br)**.

---

## ✨ Principais Recursos

- Criptografia militar de ponta a ponta: PBKDF2 (100.000 iterações) + AES-GCM 256 bits
- Arquitetura Zero-Knowledge: sua senha-mestre nunca é enviada pela rede nem gravada em disco
- Preenchimento automático in-field (auto-fill) e captura de novas credenciais
- Gerador avançado de senhas seguras e análise de vulnerabilidade

---

## 🚀 Como Instalar e Testar no Google Chrome / Chromium

1. Clone este repositório ou baixe o código-fonte:
   ```bash
   git clone git@github.com:4u-Labs/safepass-extension.git
   ```
2. Abra o Google Chrome ou qualquer navegador baseado em Chromium (Brave, Edge, Vivaldi).
3. Acesse a URL: `chrome://extensions/`
4. No canto superior direito, ative a chave **Modo do desenvolvedor**.
5. No canto superior esquerdo, clique no botão **Carregar sem compactação** (*Load unpacked*).
6. Selecione a pasta onde o repositório foi clonado.
7. A extensão estará imediatamente ativa e pronta para uso!

---

## 👨‍💻 Autor & Créditos

- **Organização:** [4u-Labs](https://github.com/4u-Labs)
- **Portal Oficial:** [4U.IA.BR](https://4u.ia.br)
- **Autor:** Fabiano Braga (ORCID: [0009-0004-5936-5060](https://orcid.org/0009-0004-5936-5060))
- **Licença:** MIT / Uso Proprietário 4U.IA.BR
